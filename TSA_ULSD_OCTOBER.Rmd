...
df_crack_spread <- HOdf %>% 
  dplyr::filter(stringr::str_detect(Contracts, "Crack"))
  
p_cs <- df_crack_spread %>% ggplot(aes(x = date, y = Price)) +
  geom_line() +
  scale_y_continuous(labels = scales::comma)
p_cs %>% plotly::ggplotly(width = 700, height = 400,)


df_flat <- HOdf %>% 
  dplyr::filter(stringr::str_detect(Contracts,c("CL22H")))

p_flat <- df_flat %>% ggplot(aes(x = date, y = Price)) +
  geom_line() +
  scale_y_continuous(labels = scales::comma)
p_flat %>% plotly::ggplotly(width = 700, height = 400,)
...
df_Inventory <- eia_df %>%
  ...
  dplyr::select(param, type, data) %>% unnest() %>% 
  dplyr::filter(
    (date>="2018-11-30")&(date<="2021-10-01")
    )
...
p_inv %>% plotly::ggplotly(width = 700, height = 400,)
...
df_crack_spread <- df_crack_spread%>% 
  mutate(
    crack_spread = log(Price/lag(Price))
  ) %>% 
  na.omit() %>% 
  select(date,crack_spread)



df_Inventory <- df_Inventory %>% 
  mutate(
    Inventory = log(value/lag(value))
  ) %>% 
  na.omit()%>%
  select(date,Inventory)  


df_Inventory <- df_Inventory %>% arrange(date)
df_crack_spread <- df_crack_spread %>% arrange(date)

df_merge <- left_join(df_crack_spread,df_Inventory) %>% 
  # arrange(date) %>% 
  tidyr::fill(Inventory) %>% 
  na.omit()


df_merge %>% ggplot(
  aes(x = crack_spread, y = Inventory)
                    )+geom_point()
...
df_merge2 <- left_join(df_crack_spread,df_supply) %>% 
  # arrange(date) %>% 
  tidyr::fill(Supply) %>% 
  na.omit()


fit <- stats::lm(crack_spread ~ Inventory, data = df_merge)
# Returns an object of type list with all info
# summary(fit), plot(fit)
summary(fit)


fit2 <- stats::lm(crack_spread ~ Supply, data = df_merge2)
# Returns an object of type list with all info
# summary(fit), plot(fit)
summary(fit2)
...
p_Storage %>% plotly::ggplotly(width = 700, height = 400,)
...
p_Supply %>% plotly::ggplotly(width = 700, height = 400,)


p_Imports <-
  data2 %>%
  ggplot(aes(x = date, y = Imports)) +
  geom_line() +
  scale_y_continuous(labels = scales::comma) +
  ggtitle('Distillate Fuel Oil U.S. Total Inventory, Monthly')

p_Imports %>% plotly::ggplotly(width = 700, height = 400,)
...
p_Demand %>% plotly::ggplotly(width = 700, height = 400,)
...
p_Exports %>% plotly::ggplotly(width = 700, height = 400,)
...
p_cs/p_Supply/p_Demand
p_cs/p_Imports/p_Exports
...
df_join_all_long <- df_join_all %>% pivot_longer(-date, names_to = "series",values_to = "value" ) 
...
```{r}
fit <- stats::lm(HO_Crack_Spd ~ spread, data = df_join_all)
summary(fit)
```
...
```{r}
fit2 <- stats::lm(HO_Apr22_Mar22_Spd ~ spread, data = df_join_all)
summary(fit2)
```
